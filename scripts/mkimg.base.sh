...
build_apkovl() {
	...
	for _script in "$PWD"/"$2" $HOME/.mkimage/$2 \
		$(readlink -f "$scriptdir/$2"); do

		if [ -f "$_script" ]; then
			break
		fi
	done
	[ -n "$_script" ] || die "could not find $2"
	(cd "$DESTDIR"; fakeroot "$_script" "$_host")
}

section_apkovl() {
	for a in $(echo $apkovl | xargs) ; do
		[ -n "$a" -a -n "$hostname" ] || return 0
		build_section apkovl $hostname $a $(checksum < "$a")
	done
}
...
profile_base() {
	...
	apkovl="genapkovl-base.sh genapkovl-repos.sh genapkovl-motd.sh genapkovl-installer.sh"
	hostname="alpine"
}
